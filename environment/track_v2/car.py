...
class AbstractCar:
    ...
    Base class for Car
    """
    def __init__(self, img_path, max_vel=5, rotation_vel=5):
        self.img = pygame.image.load(img_path)
        self.max_vel = max_vel
        self.vel = 0
        self.rotation_vel = rotation_vel
        self.angle = 0
        self.START_POS = (90, 290)  # Default start position, to be set later
        self.x, self.y = self.START_POS
        self.acceration = 0.1

    def rotate(self, steering_angle):
        self.angle += steering_angle * self.rotation_vel
    ...
    def step(self, action):
        # action[0]: Acceleration (float between -1 and 1), positive is forward, negative is backward
        # action[1]: Steering angle (float between -1 and 1), controls left/right turn
        accel, steering_angle = action

        if accel > 0:
            self.vel = min(self.vel + self.acceration * accel, self.max_vel)
        elif accel < 0:
            self.vel = max(self.vel + self.acceration * accel, -self.max_vel / 2)
